...
function startGame(cxt) {
    ...
    // 新建两个初始方块,改为新建10个初始方块
    newBox(cxt);
    newBox(cxt);
    newBox(cxt);
    newBox(cxt);
    newBox(cxt);
    newBox2(cxt);//生成数字16和32的方块
    newBox2(cxt);
    newBox2(cxt);
    newBox2(cxt);
    newBox2(cxt);
...
function newBox(cxt) {
    ...
    // 方块的数值随机2或4;改为数值随机4或8
    var randNumber = Math.random() < 0.5 ? 8 : 16;
...
function newBox2(cxt) {
    ...
    // 随机生成一个位置（x:0-3,y:0-3）
    var randx2 = parseInt(Math.floor(Math.random() * 4))
    var randy2 = parseInt(Math.floor(Math.random() * 4))
    ...
    var times = 0
    while (times < 50) {
        if (nums[randx2][randy2] == 0) {
            break
        }

        randx2 = parseInt(Math.floor(Math.random() * 4))
        randy2 = parseInt(Math.floor(Math.random() * 4))
    ...
    // 方块的数值随机16或32
    var randNumber2 = Math.random() < 0.4 ? 16 : 32;
    // randx2,randy2 位置的方框赋上刚才的随机初始值
    nums[randx2][randy2] = randNumber2;

    // 绘制方块
    drawBox(cxt, randx2, randy2, randNumber2);
