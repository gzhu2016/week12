...
function newBox(cxt) {
    ...
    while (times < 50) {
        ...
        randx = parseInt(Math.floor(Math.random() * 6))
        randy = parseInt(Math.floor(Math.random() * 6))
        times++
    }
            ...
            if (nums[i][j] != 0) {
                ...
                for (var k = 0; k < i; k++) {
                    ...
                    // 并且记分，分值就是原本数值的2倍，再刷新分数
                        ...
                        nums[k][j] += nums[k][j]
                        nums[i][j] = 0
                        score += (Math.log(nums[k][j])/Math.log(2))
                        updateScore(score)
                    }
                }
            }
            ...
            if (nums[i][j] != 0) {
                for (var k = 0; k < j; k++) {
                    if (nums[i][k] == 0) {
                        ...
                        nums[i][k] += nums[i][j]
                        nums[i][j] = 0
                        score += (Math.log(nums[i][k])/Math.log(2))
                        updateScore(score)
                    }
                }
            }
...
function updateScore(score) {
    document.getElementById("score").innerText = score.toString(16).toLocaleUpperCase()
}
